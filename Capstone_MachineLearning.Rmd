...
inTrain <- createDataPartition(y=bankfull_mod$subscribed, p=0.75, list=FALSE)
trainData <- bankfull_mod[ inTrain,]
testData  <- bankfull_mod[-inTrain,]
...
glm_model_test <- train(subscribed ~ age + job + marital + edu_lvl, data=trainData, method="glm", na.action=na.omit)


glm_model_test <- train(subscribed ~ ., data=trainData, method="glm", na.action=na.omit)
...
```{r}
# Which model is best?
glm_prediction <- predict(glm_model_full, newdata=testData)
#glm_prediction <- predict(glm_model_dur, newdata=testData)
...
and specificity.
```{r}
glm_confMatrix <- caret::confusionMatrix(glm_prediction, testData$subscribed, mode="everything") 
