...
```{r}
bankfull_clean <- read_csv("bankfull_clean.csv", col_types = "ifffffffffiiiicdddddfl")
```
...
# develop a model
#glm_model <- train(subscribed ~ ., data=trainData, method="glm", na.action=na.omit)
glm_model <- train(subscribed ~ duration_sec, data=trainData, method="glm", na.action=na.omit)
...
```{r}
glm_prediction <- predict(glm_model, newdata=testData)
```
