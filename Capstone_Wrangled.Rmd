...
_Although most of the sales from the previous campaign resulted in failed 
attempts, the larger number of `nonexistent` results makes this variable 
useless._
```{r}
ggplot(bankfull, aes(x = subscribed, fill = prev_outcome)) + 
  geom_bar(position="fill") + 
  labs(title="Proportion of subscriptions from previous campaign relative to current campaign")

prop.table(table(bankfull$prev_outcome))
```

If the `nonexistent` values were removed, more than half the clients that 
subscribed previously will subscribe again.
```{r}
bankfull_prev <- filter(bankfull, prev_outcome != "nonexistent")
ggplot(bankfull_prev, aes(x=subscribed, fill=prev_outcome)) + 
  geom_bar(position="fill") + 
...
freq(bankfull %>% filter(subscribed == "yes") %>% select(month))
...
```{r}
b_soceco <- select(bankfull, emp_var_rate:num_employed)
ggpairs(b_soceco)
```
