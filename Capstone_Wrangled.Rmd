...
personal loan) also do not express any trends with respect to subscription.
```{r}
...
descr(bankfull$duration_min)
ggplot(bankfull, aes(x=duration_min, fill=subscribed)) + geom_histogram(bins=10)
ggplot(bankfull, aes(x=duration_min, fill=subscribed)) + geom_histogram(bins=10, position="fill")

#time_to_subscribed <- bankfull %>% filter(subscribed="yes")
