...
With the `nonexistent` values removed, notable more than half the clients can 
...
do not seem to show any strong trends.
```{r}
...
ggplot(bankfull, aes(x=duration_sec, fill=subscribed)) + geom_histogram(bins=10, position="fill")

ggplot(bankfull, aes(x=subscribed, y=duration_sec)) + 
  geom_boxplot(outlier.colour="blue", outlier.shape=1, outlier.size=2)
```

Let's focus on the duration of the last contact for those who subscribed.
```{r}
time_to_subscribed <- filter(bankfull, subscribed=="yes")
descr(time_to_subscribed$duration_sec)
ggplot(time_to_subscribed, aes(x=subscribed, y=duration_sec)) + geom_boxplot()
...
ggplot(bankfull, aes(x=contact_typ, fill=subscribed)) + 
  geom_bar(position="fill")
xtabs(~contact_typ + subscribed, data=bankfull)
```

It looks somewhat safe to say the chance of a susbscription occuring decreases 
with each additional contact. The bar graph indicates clients will not 
subscribe if contacted more than 11 times.
```{r}
ggplot(bankfull, aes(x=contact_cnt, fill=subscribed)) + geom_bar(position="fill")
xtabs(~contact_cnt + subscribed, data=bankfull)
```
...
But if we stack each month as relative proportions we can see customers are 
...
ggpairs(bankfull, columns=16:20)
...
```{r}
ggplot(bankfull, aes(emp_var_rate, euribor3m)) + geom_point() + facet_wrap(~subscribed)
```


```{r}
reject <- filter(bankfull, subscribed == "no")
accept <- filter(bankfull, subscribed == "yes")
summary(reject$duration_sec)
summary(accept$duration_sec)
ggplot(bankfull, aes(subscribed, duration_sec)) + geom_boxplot()
#outlier < Q1 - 1.5 x IQR; #255.5 - 1.5(751 - 255.5)
#outlier > Q3 + 1.5 x IQR; #1519.25 > 761 + 1.5(505.5)
# filtering out the outliers for further investigation
acceptOutlier <- filter(accept, subscribed == "yes" & duration_sec <= 1519.25)
ggplot(acceptOutlier, aes(subscribed, duration_sec)) + geom_boxplot() + 
    labs(title = "Boxplot with Outliers removed")
ggplot(acceptOutlier, aes(duration_sec)) + geom_histogram(bins=10)
#ggplot(acceptOutlier, aes(duration_sec)) + geom_bar()
...
Box-plots for the employment variable rate and the 3 month Euribor rating with 
respect to subscriptions look very similar.
```{r}
...
6. Can you make a time-series plot? NOPE
