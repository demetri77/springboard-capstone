...
Hypothesis: removing the "unknown" values should help ML later.
```{r}
bankfull <- filter(bankfull, default != "unknown" & housing != "unknown" & loan != "unknown")
```
...
The most subscriptions do occur in *May* and other summer months, *June, July, 
and August* as confirmed by the frequency chart. Winter months *January* and 
...
ctable(daysPassedMOD$days_passed, daysPassedMOD$subscribed)
ggplot(daysPassedMOD, aes(x=days_passed, fill=subscribed)) + geom_histogram(binwidth=1, position="fill")

```

There isn't data for 8 and 20 days respectively.
...
**Drawing Board**
```{r}
ggplot(bankfull, aes(x=emp_var_rate, fill=subscribed)) + geom_histogram()
#ctable(bankfull$emp_var_rate, bankfull$subscribed)
ggplot(bankfull, aes(x=cons_price_idx, fill=subscribed )) + geom_histogram()
ggplot(bankfull, aes(x=cons_conf_idx, fill=subscribed)) + geom_histogram()
ggplot(bankfull, aes(x=euribor3m, fill=subscribed)) + geom_histogram()
#ctable(bankfull$euribor3m, bankfull$subscribed)
ggplot(bankfull, aes(x=num_employed, fill=subscribed)) + geom_histogram()
```
...
2 - For clients that do subscribe, twice as many do so via mobile phone than 
  regular telehone.
X - Wednesdays are a good day to speak with clients about subscribing.
4 - It is advised not to contact a client more than 11 times.
5 - The length of the call should be more than 274 seconds or 4 min, 34 sec.
