...
subscribe if contacted by more than the maximum count value.
```{r}
ggplot(bankfull, aes(x=contact_cnt, fill=subscribed)) + geom_bar(position="fill")
maximum_contact_cnt <- max(bankfull %>% filter(subscribed=="yes") %>% select(contact_cnt))
maximum_contact_cnt
```
...
Further investigation of the duration_sec factor.
```{r}
