...
Last contact data will include details about the last contact with the client. Variables includ type of contact, the day and month of contact, and the 
...
*June, July, and August* as confirmed by the frequency chart. Winter months 
*January* and *February* are not a part of the dataset.
...
When we integrate weekdays with the monthly values, the second column, 
*Wednesday*, becomes a fairly good day to subscribe.
...
The variable _duration_ refers to the length of time in seconds spent with the 
...
#durMinYES <- subscribedYES$duration_sec/60
subscribedNO <- filter(bankfull, subscribed=="no") %>% select(duration_sec)
#durMinNO <- subscribedNO$duration_sec/60
descr(subscribedYES)
descr(subscribedNO)
```

There's a slight overlap in the Interquaritle Range of subscribers with the 
non-subscribers. let's focus on the duration of the last contact for those who 
subscribed.

Taking note the duration of the last contact for That is, there's a cut-off from the Q3 for non-subscribers and Q1 of subscribers.
...
The following density plot for the number of days that passed by after the client was last contacted from a previous campaign does not indicate any special trends. Note that a value of 999 means client was not previously contacted.

```{r}
ggplot(bankfull, aes(days_passed)) + geom_histogram(bins=10)

freq(bankfull$days_passed)
ctable(bankfull$days_passed, bankfull$subscribed)

ggplot(bankfull, aes(x=prev_contact_cnt)) + geom_density() + facet_wrap(~subscribed)
```
